...
try {
	...
	if(file_exists(dirname(dirname(__FILE__)).'/recovery.php')) {
		throw new Exception('recovery.php exists in VirtualBox Web Manager\'s folder. This is a security hazard. VirtualBox Web Manager will not run until recovery.php has been renamed to a file name that does not end in .php such as <b>recovery.php-disabled</b>.',vboxconnector::VBWEBMGR_ERRNO_FATAL);
	}

	/* Check for PHP version */
	if (!version_compare(PHP_VERSION, '5.2.0', '>=')) {
		throw new Exception('VirtualBox Web Manager requires PHP >= 5.2.0, but this server is running version '. PHP_VERSION .'. Please upgrade PHP.');
	}
	...
	switch($request['fn']) {
		...
		/*
		 * Return vboxwebmgr's configuration data
		 */
		case 'getConfig':
			...
			$response['data']['responseData']['groupDefinitionKey'] = ($settings->vboxwebmgrGroups ? vboxconnector::vboxwebmgrGroupKey : 'GUI/GroupDefinitions');
		...
		/*
		 * Change vboxwebmgr password. Passed to auth module's
		...
		/*
		 * Get a list of vboxwebmgr users. Passed to auth module's
		 * getUsers method.
		 */
		case 'getUsers':
		...
		/*
		 * Remove a vboxwebmgr user. Passed to auth module's
		 * deleteUser method.
		 */
		case 'delUser':
		...
		/*
		 * Edit a vboxwebmgr user. Passed to auth module's
		 * updateUser method.
		 */
		case 'editUser':
		...
		/*
		 * Add a user to vboxwebmgr. Passed to auth module's
		 * updateUser method.
		 */
		case 'addUser':
		...
		/*
		 * Log out of vboxwebmgr. Passed to auth module's
		 * logout method.
		 */
		case 'logout':
...
if($vbox && $vbox->errors) {
	...
	foreach($vbox->errors as $e) { /* @var $e Exception */
		...
		if($e->getCode() == vboxconnector::VBWEBMGR_ERRNO_CONNECT && isset($vbox->settings))
			$d .= "\n\nLocation:" . $vbox->settings->location;

		$response['messages'][] = htmlentities($e->getMessage()).' ' . htmlentities($details);

		$response['errors'][] = array(
			'error'=> ($e->getCode() & vboxconnector::VBWEBMGR_ERRNO_HTML ? $e->getMessage() : htmlentities($e->getMessage())),
			'details'=>htmlentities($d),
			'errno'=>$e->getCode(),
			// Fatal errors halt all processing
			'fatal'=>($e->getCode() & vboxconnector::VBWEBMGR_ERRNO_FATAL),
			// Connection errors display alternate servers options
			'connection'=>($e->getCode() & vboxconnector::VBWEBMGR_ERRNO_CONNECT)
		);
	}
}
