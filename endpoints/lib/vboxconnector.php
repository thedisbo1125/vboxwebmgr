...
class vboxconnector {
	 ...
	 * @var integer
	 */
	const VBWEBMGR_ERRNO_HTML = 1;

	/**
	 * Error number describing a fatal error
	 * @var integer
	 */
	const VBWEBMGR_ERRNO_FATAL = 32;

	/**
	 * Error number describing a connection error
	 * @var integer
	 */
	const VBWEBMGR_ERRNO_CONNECT = 64;

	/**
	 * vboxwebmgr groups extra value key
	 * @var string
	 */
	const vboxwebmgrGroupKey = 'phpvb/Groups';
	...
	/**
	 * Settings object
	 * @var vboxwebmgrConfigClass
	 * @see vboxwebmgrConfigClass
	 */
	var $settings = null;
	...
	public function __construct($useAuthMaster = false) {
		...
		/** @var vboxwebmgrConfigClass */
		$this->settings = new vboxwebmgrConfigClass();

		// Are default settings being used?
		if(@$this->settings->warnDefault) {
			throw new Exception("No configuration found. Rename the file <b>config.php-example</b> in vboxwebmgr's folder to ".
					"<b>config.php</b> and edit as needed.<p>For more detailed instructions, please see the installation wiki on ".
					"vboxwebmgr's web site. <p><a href='https://github.com/thedisbo1125/vboxwebmgr/wiki' target=_blank>".
					"https://github.com/thedisbo1125/vboxwebmgr/wiki</a>.</p>",
						(vboxconnector::VBWEBMGR_ERRNO_FATAL + vboxconnector::VBWEBMGR_ERRNO_HTML));
		}

		// Check for SoapClient class
		if(!class_exists('SoapClient')) {
			throw new Exception('PHP does not have the SOAP extension enabled.',vboxconnector::VBWEBMGR_ERRNO_FATAL);
		}
	...
	public function connect() {
		...
		if(!@$this->skipSessionCheck && !$_SESSION['valid']) {
			throw new Exception(trans('Not logged in.','UIUsers'),vboxconnector::VBWEBMGR_ERRNO_FATAL);
		}
		...
		try {
			...
			throw new Exception($msg,vboxconnector::VBWEBMGR_ERRNO_CONNECT);
		}


		// Error logging in
		if(!$this->vbox->handle) {
			throw new Exception('Error logging in or connecting to vboxwebsrv.',vboxconnector::VBWEBMGR_ERRNO_CONNECT);
		}
			...
			switch($event['eventType']) {
				...
				case 'OnMachineDataChanged':
					...
					try {
						...
						if($this->settings->vboxwebmgrGroups) {
							$groups = explode(',',$machine->getExtraData(vboxconnector::vboxwebmgrGroupKey));
	...
	function __call($fn,$args) {
		...
		// Not found
			...
			throw new Exception('Undefined method: ' . $fn ." - Clear your web browser's cache.",vboxconnector::VBWEBMGR_ERRNO_FATAL);
	...
	public function remote_machinesSaveGroups($args) {
		...
		foreach($args['vms'] as $vm) {
			...
			if($this->settings->vboxwebmgrGroups) {
				...
				$oldGroups = explode(',',$machine->getExtraData(vboxconnector::vboxwebmgrGroupKey));
			...
			try {
				...
				if($this->settings->vboxwebmgrGroups) {
					$this->session->machine->setExtraData(vboxconnector::vboxwebmgrGroupKey, implode(',', $newGroups));
				} else {
			...
			try {
				...
				for($i = 0; $i < $max; $i++) {
					...
					if(@serialize($deviceFilters[$i]) != @serialize($args['USBDeviceFilters'][$i])) {
						...
						// Exists in new?
						if(is_array($args['USBDeviceFilters'][$i])){
							if(count($args['USBDeviceFilters'][$i])) {
	...
	public function remote_machineCreate($args) {
		...
		$filename = $this->vbox->composeMachineFilename($args['name'],($this->settings->vboxwebmgrGroups ? '' : $args['group']),$this->vbox->systemProperties->defaultMachineFolder,null);
		...
		if($this->settings->vboxwebmgrGroups && $args['group']) {
			$m->setExtraData(vboxconnector::vboxwebmgrGroupKey, $args['group']);
		}
	...
	private function _machineGetDetails(&$m) {
		...
		if($this->settings->vboxwebmgrGroups) {
			$groups = explode(',',$m->getExtraData(vboxconnector::vboxwebmgrGroupKey));
			if(!is_array($groups) || (count($groups) == 1 && !$groups[0])) $groups = array("/");
		} else {
			$groups = $m->groups;
		}
	...
	public function remote_vboxGetComposedMachineFilename($args) {
		...
		return $this->vbox->composeMachineFilename($args['name'],($this->settings->vboxwebmgrGroups ? '' : $args['group']),$this->vbox->systemProperties->defaultMachineFolder,null);
	...
	*/
	...
	public function remote_vboxGroupDefinitionsSet($args) {
		...
		$groupKey = ($this->settings->vboxwebmgrGroups ? vboxconnector::vboxwebmgrGroupKey : 'GUI/GroupDefinitions');
