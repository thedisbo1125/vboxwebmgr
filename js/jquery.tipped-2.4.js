 ...
 *
 *
 * Additional changes added by thedisbo1125, https://github.com/thedisbo1125
 * project was archived on google code with last change made 3/1/2011
 *
 *
 * Compatibility: Tested with jQuery 1.4.2+, though it should work with 1.3.x
 *
 * v2.4     - switch deprecated event handlers to .on( and trigger(
 * v2.3d    - Remove data from element tip is cleared on
 *          - Switch from jquery "unbind" to javascript "removeEventListener" as unbind removed all event handlers from element, not
 *              just specific ones for tipped
    ...
    window.Tip = {
        ...
        hide:function(immediate){
            ...
            if(immediate){
                ...
                this.$tip.on('mouseenter',function(){ clearTimeout(Tip.timer_hide); });

                /* Once the user hovers out of the tip, hide it immediately */
                this.$tip.on("mouseleave",function(){ Tip.hide(true); });
         ...
         */
        removetip:function(){
            $target[0].removeEventListener("mouseover", this.OnMouseOverSetup);
            $target[0].removeEventListener("mousemove", this.OnMouseMoveSetup);
            $target[0].removeEventListener("mouseleave", this.OnMouseLeaveSetup);
        },
        ...
        */
        ...
        _setSize:function(){
            ...
            if(this.$tip.outerHeight() > $(window).height()){
                ...
                if(this.settings.oversizeStick){
                    ...
                    this.$target[0].removeEventListener('mouseout');
                }
        ...
        _calcInitialPosition:function(Pos){
            switch(this.settings.position){
                case 'mouse':
                    Pos.x = this.mouseloc.currx + this.settings.marginX;
                    Pos.y = this.mouseloc.curry + this.settings.marginY;
                    break;
                case 'absolute':
    ...
    $.fn.tipped = function(settings){
        this.each(function(i){
            ...
            if(!settings.clear){
                ...
                $target.removeData('tipped');

                // remove event handlers setup when tip initialized
                $target[0].removeEventListener( "mouseover", Tip.OnMouseOverInit);
                $target[0].removeEventListener("mousemove", Tip.OnMouseLeaveInit);
                $target[0].removeEventListener("mouseleave", Tip.OnMouseClickInit);
