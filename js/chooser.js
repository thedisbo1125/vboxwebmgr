...
var vboxChooser = {
	...
	/* Check vboxwebmgr version and VirtualBox
	...
	vmGroupDropped : function(e, droppedGroup) {
		...
		if(dropTarget[0]) {
			...
			if(dropTarget.closest('div.vboxChooserGroup').parent().closest('div.vboxChooserGroup').data('vmGroupPath') != currParentGroupPath) {
				...
				if(sessionLocked && !$('#vboxPane').data('vboxConfig')['vboxwebmgrGroups']) return;
	...
	vmDropped : function (e, droppedVM){
		...
		if(dropTarget[0]) {
			...
			if(thisSessionLocked && !$('#vboxPane').data('vboxConfig')['vboxwebmgrGroups'] && ($(droppedVM).closest('div.vboxChooserGroup').data('vmGroupPath') != dropTarget.closest('div.vboxChooserGroup').data('vmGroupPath'))) {
				return
			}
		...
		// Not dropped above / below vm
			...
			if(thisSessionLocked && !$('#vboxPane').data('vboxConfig')['vboxwebmgrGroups']) return;
